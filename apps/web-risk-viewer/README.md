...
`public/data/` already ships with real generated data, committed in this
repo - no pipeline run needed just to view the app. To regenerate it, see
the separate `45-45-platform` repo (private, sibling checkout) - its
`data_pipelines/pipelines/08_prepare_webapp_data.py` (and friends) publish
straight into this repo's `public/data/`, resolving this repo's location via
a `WEB_REPO_DIR` env var (defaults to a sibling `45-45-Days` checkout).
...
comes from static files under `public/data/`, generated by
`45-45-platform`'s `data_pipelines/pipelines/08_prepare_webapp_data.py`
(and, for the Rapid Response tab, scripts `09`, `10`, `16`-`18`):
...
src/
  main.tsx, App.tsx
  components/
    Risk Viewer:     MapView, ControlPanel, Legend, CountyInfoPanel,
                      ProductSelector, CropSelector, WeekSelector,
                      CountySearch, MapLayerControl (base map + opacity,
                      floats on the map itself)
    Rapid Response:   ModeSwitcher, DamageSubTabSwitcher, CropHealthControls,
                      CropHealthPanel, CropHealthMapColumn,
                      CropLossProbabilityControls, CropLossProbabilityPanel
    Shared:           FieldTrendPanel, TrendChart
  config/         - products.ts (the registry), legends.ts (color ramps)
  state/          - AppStateContext.tsx (crop/product/week/reference/etc.)
  utils/          - colorScales.ts, formatters.ts, leafletLayers.ts,
                    dataLoader.ts, basePath.ts (GitHub Pages subpath
                    handling), pdfReport.ts, choroplethPdf.ts
  types/          - products.ts, damage.ts (all shared TS interfaces)
```
